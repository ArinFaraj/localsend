...
class ReceiveController {
      ...
      if (message != null) {
        ...
        if (settings.copyToClipboard) {
          try {
            ...
            _logger.info('Copied message to clipboard');
          } catch (e) {
            _logger.warning('Failed to copy message to clipboard', e);
          }
        }
        await server.ref
  ...
  Future<void> _uploadHandler({
    ...
    try {
      ...
      // Copy to clipboard if enabled and file is appropriate type
      final settings = server.ref.read(settingsProvider);
      if (settings.copyToClipboard && filePath != null && filePath.isNotEmpty) {
        final fileType = receivingFile.file.fileType;
        final mime = receivingFile.file.lookupMime();

        bool copied = false;
        
        if (fileType == FileType.text || mime.startsWith('text/') || mime == 'application/json' || mime == 'application/xml') {
          try {
            final file = File(filePath);
            final textContent = await file.readAsString();
            await Clipboard.setData(ClipboardData(text: textContent));
            _logger.info('Copied ${receivingFile.desiredName} to clipboard');
            copied = true;
          } catch (e) {
            _logger.warning('Failed to copy file as text to clipboard', e);
          }
          ...
          try {
            ...
            _logger.info('Copied image ${receivingFile.desiredName} to clipboard');
            copied = true;
          } catch (e) {
            _logger.warning('Failed to copy image to clipboard', e);
          }
        }
        ...
        if (!copied) {
          try {
            ...
            _logger.info('Copied file path ${receivingFile.desiredName} to clipboard as fallback');
          } catch (e) {
            _logger.warning('Failed to copy file path to clipboard', e);
          }
        }
      }
